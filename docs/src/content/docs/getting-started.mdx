...
1. ` @six-tech/starlight-theme-six` is a Starlight [plugin](https://starlight.astro.build/reference/plugins/). Install it by running the following command in your terminal:
   ...
   ```sh
   npm install @six-tech/starlight-theme-six @fontsource/inter @fontsource/jetbrains-mono sharp
   ```
   ...
   ```sh
   pnpm add @six-tech/starlight-theme-six @fontsource/inter @fontsource/jetbrains-mono sharp
   ```
   ...
   ```sh
   yarn add @six-tech/starlight-theme-six @fontsource/inter @fontsource/jetbrains-mono sharp
   ```
   ...
   - **`@fontsource/inter` & `@fontsource/jetbrains-mono`**: For theme fonts
   - **`sharp`**: For image processing (required by Astro for optimizing images)
   
   These are peer dependencies, meaning you must install them explicitly in your project.
   
   **Troubleshooting Sharp Installation:**
   
   If you encounter "Could not find Sharp" errors after installation, Sharp's native binaries may need to be rebuilt for your platform. Try:
   
   ```bash
   # Force rebuild Sharp's native binaries
   ...
   This forces Sharp to recompile its native binaries for your specific platform (Windows/macOS/Linux).
   :::
